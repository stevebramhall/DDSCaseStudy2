...
# Recode Attrition with 0s and 1s
alldata$Attrition <- ifelse(alldata[,3] == "Yes", 1, ifelse(alldata[,3] == "No", 0, 99))
...
```{r}
samplesize=nrow(alldata)
train_percent = .6
train_indices = sample(seq(1,samplesize,length = samplesize),train_percent*samplesize) # get random indices
training = alldata[train_indices,c(1,2)]   # random training data from original data
test = alldata[-train_indices,c(1,2)]      # random test data from original data
training.attrition = alldata[train_indices,3]
test.attrition = alldata[-train_indices,3]

#### Used the class package’s knn function to build an KNN classifier.
knn.k21 = knn(training,test,cl=training.attrition,k=21)
